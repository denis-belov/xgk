...
namespace XGK {
  ...
  namespace VULKAN {
    ...
    uint32_t getMemTypeIndex (Context* context, VkMemoryPropertyFlags flags) {

      uint32_t mem_type_count = context->mem_props.memoryTypeCount;

      for (uint32_t i = 0; i < mem_type_count; i++) {

        uint32_t match = (context->mem_reqs.memoryTypeBits & (1 << i)) && (mem_props->memoryTypes[i].propertyFlags & flags);
